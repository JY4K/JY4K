...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        hdata = self.fetch(f"{self.host}/api/dyTag/hand_data", params={'category_id': cdata['data'][0]['id']},
    ...
    def detailContent(self, ids):
        ...
        if ids[-1] == '67':
            ...
            for i in v.get('source_list_source', []):
                n.append(i.get('name'))
                p.append('#'.join(
                    [f"{j.get('source_name') or j.get('weight')}${j['url']}" for j in i.get('source_list', [])]))
    ...
    def searchContent(self, key, quick, pg="1"):
        data = self.fetch(f"{self.host}/api/v2/search/videoV2", params={'key': key, 'page': pg, 'pageSize': 20},
                          headers=self.headers).json()
